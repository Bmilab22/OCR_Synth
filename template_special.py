...
import numpy as np
from elements import Background_inter, Document_inter
...
class SynthDoG(templates.Template):
    def __init__(self, config=None, split_ratio: List[float] = [0.8, 0.1, 0.1]):
        ...
        self.background = Background_inter(config.get("background", {}))
        self.document = Document_inter(config.get("document", {}))
        ...
        # 配置文本数量范围
        self.text_count_config = config.get("document", {}).get("text_count", {"min": 2, "max": 50})
        ...
        self.shape = components.Switch(
            components.Selector(
                [
                    components.ElasticDistortion(**config.get("shape", {}).get("elastic_distortion_1", {})),
                    components.ElasticDistortion(**config.get("shape", {}).get("elastic_distortion_2", {}))
                ]
            ),
            prob=config.get("shape", {}).get("prob", 0.5),  # 默认概率为0.5
            args=config.get("shape", {}).get("args", {})
        )
    ...
    def save(self, root, data, idx):
        ...
        roi = data["roi"]
        bboxes = data["bboxes"]
        page_bbox = data.get("page_bbox", None) 

        output_dirpath = root  # 所有文件保存在同一个目录
        
        # 检查 idx 是否已存在，若存在则跳过或调整 idx
