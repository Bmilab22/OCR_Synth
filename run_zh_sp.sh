...
while read line; do
  ...
  # 创建一个基于PID的唯一临时文件
  temp_file="config_zh_ocr_sp_temp_$$"

  # 复制一份原始的yaml文件
  cp config_zh_ocr_sp.yaml $temp_file.yaml
  ...
  # 忽略错误并运行srun命令
  set +e
  synthtiger -o "./zh-noise/1M/$filename" -c 20000 -w 4 -v template_special SynthDoG $temp_file.yaml \
